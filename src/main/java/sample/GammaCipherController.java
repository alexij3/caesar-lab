...
import sample.cipher.gamma.GammaCipher;
import sample.cipher.gamma.Key;
...
public class GammaCipherController {
    ...
    @FXML
    private Spinner<Integer> firstKeyValue;

    @FXML
    private Spinner<Integer> secondKeyValue;

    @FXML
    private Spinner<Integer> thirdKeyValue;
    ...
    void decrypt(MouseEvent event) {
        ...
        int a = this.firstKeyValue.getValue();
        int b = this.secondKeyValue.getValue();
        int c = this.thirdKeyValue.getValue();

        Cipher cipher = new GammaCipher(new Alphabet('E', this.alphabetChars), new Key(a, b, c));
    ...
    void initialize() {
        ...
        this.firstKeyValue.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                this.firstKeyValue.increment(0); // won't change value, but will commit editor
            }
        });
        this.secondKeyValue.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                this.secondKeyValue.increment(0); // won't change value, but will commit editor
            }
        });
    }
